...
# Colour palette matching screenshot 2 style
ACCENT      = "#29A9E0"   # Telegram blue
ACCENT_HOV  = "#1A8BBF"
...
# ---------------------------------------------------------------------------
# Tabler Icons unicode codepoints
# ---------------------------------------------------------------------------
TI = {
    "home":         chr(60097),
    "key":          chr(60103),
...
def _ti(icon: str, size: int = 14) -> tuple[str, tuple]:
    """Return (char, font_tuple) for a Tabler icon."""
    _load_tabler_font()
    fallbacks = {"home": "⌂", "key": "⚿", "settings": "⚙",
                 "info-circle": "ℹ", "copy": "⎘", "download": "↓", "brand-github": "◉"}
    if _tabler_loaded:
        return TI.get(icon, "?"), ("tabler-icons", size)
    return fallbacks.get(icon, "•"), ("", size)
...
def find_chromium_executable() -> str | None:
    ...
    if not playwright_dir.exists():
        return None
    for d in sorted(playwright_dir.glob("chromium-*"), reverse=True):
...
class TelegramExtractor:
    ...
    def _extract(self) -> dict:
        ...
        keys    = re.findall(r"(-----BEGIN RSA PUBLIC KEY-----.*?-----END RSA PUBLIC KEY-----)", text, re.S)
...
def _copy_btn(parent, get_value):
    """Small icon copy button."""
    char, font = _ti("copy", 13)
    b = ctk.CTkButton(
        parent, text=char, font=font if _tabler_loaded else _sf(11),
        width=28, height=22,
        fg_color="transparent",
        hover_color=("#2a2d35", "#2a2d35"),
        text_color=("gray60", "gray60"),
        command=lambda: (pyperclip.copy(get_value()), None)
    )
    return b
...
def _data_card(parent, label: str, value: str, col: int, row: int):
    ...
    _lbl(top, label, 11, color=("gray55", "gray55")).grid(row=0, column=0, sticky="w")
    _copy_btn(top, lambda v=value: v).grid(row=0, column=1, sticky="e")

    _lbl(card, value, 13, bold=True).grid(row=1, column=0, sticky="w", padx=12, pady=(2, 10))
    return card


def _key_card(parent, label: str, value: str, col: int, row: int):
    """Card for RSA key with monospace text and copy button."""
    card = ctk.CTkFrame(parent, corner_radius=8, fg_color=CARD_BG)
    card.grid(row=row, column=col,
...
class NavButton(ctk.CTkButton):
    def __init__(self, parent, label: str, icon: str, cmd, **kw):
        char, _ = _ti(icon, 14)
        super().__init__(
    ...
    def set_active(self, on: bool):
        if on:
            ...
            self.configure(fg_color="transparent", text_color=("gray60", "gray60"))
...
class MainTab(ctk.CTkFrame):
    def __init__(self, parent, extractor, on_ready):
    ...
    def _build(self):
        ...
        row1.grid(row=2, column=0, sticky="ew", padx=28, pady=(0, 10))
        row1.grid_columnconfigure(0, weight=1)
        self.phone = ctk.CTkEntry(row1, placeholder_text="+79991234567", height=38)
        self.phone.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.btn_get = ctk.CTkButton(
            row1, text="Получить код", width=140, height=38,
            fg_color=ACCENT, hover_color=ACCENT_HOV,
            command=self._get_code)
        self.btn_get.grid(row=0, column=1)

        row2 = ctk.CTkFrame(self, fg_color="transparent")
        row2.grid(row=3, column=0, sticky="ew", padx=28, pady=(0, 16))
        row2.grid_columnconfigure(0, weight=1)
        self.code = ctk.CTkEntry(row2, placeholder_text="Код из Telegram", height=38)
        self.code.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.btn_ok = ctk.CTkButton(
            row2, text="Подтвердить", width=140, height=38,
            fg_color=ACCENT, hover_color=ACCENT_HOV,
        ...
        self.tog = ctk.CTkButton(
            ...
            fg_color="transparent", hover_color=("#252830", "#252830"),
    ...
    def _get_code(self):
        ...
        threading.Thread(target=self._t_get, args=(phone,), daemon=True).start()

    def _t_get(self, phone):
        try:
            self._ext.send_code(phone)
        except Exception as e:
            self.log(f"[!] Ошибка: {e}")
        finally:
...
class DataTab(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # placeholder frames — filled in update_data
    ...
    def update_data(self, data: dict):
        ...
        for f in (self._app_frame, self._srv_frame, self._key_frame, self._exp_frame):
            for w in f.winfo_children():
                w.destroy()

        # — App configuration cards
        _data_card(self._app_frame, "App api_id",   str(data["api_id"]),   0, 0)
        _data_card(self._app_frame, "App api_hash",     data["api_hash"],  1, 0)
        _data_card(self._app_frame, "App title",        data.get("app_title", "—"),   0, 1)
        _data_card(self._app_frame, "Short name",       data.get("short_name", "—"),  1, 1)

        # — Server cards
        _data_card(self._srv_frame, "Test server",       data.get("test_server", "—"),       0, 0)
        _data_card(self._srv_frame, "Production server", data.get("production_server", "—"), 1, 0)

        # — Public key cards
        keys = data.get("public_keys", [])
        for i, key in enumerate(keys[:2]):
            _key_card(self._key_frame, f"Public key {i+1}", key, i, 0)
        ...
        for lbl, cmd in fmts:
            ctk.CTkButton(
                self._exp_frame, text=lbl, command=cmd,
                width=72, height=30,
                fg_color="transparent",
    ...
    def _full_text(self) -> str:
        ...
        k1 = keys[0].strip() if len(keys) > 0 else ""
    ...
    def _save(self, ext: str, content: str):
        OUTPUT_DIR.mkdir(exist_ok=True)
    ...
    def _ec(self):
        ...
        for k, v in [("api_id", self._data["api_id"]),
                     ...
                     ("production_server", self._data.get("production_server", ""))]:
            w.writerow([k, v])
...
class SettingsTab(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        for i, (name, desc, default) in enumerate(rows):
            ...
            sw = ctk.CTkSwitch(r, text="", width=46, button_color=ACCENT,
                               progress_color=ACCENT)
            sw.grid(row=0, column=1)
            if default:
                sw.select()
            ctk.CTkFrame(self, height=1, fg_color=("gray20", "gray20")).grid(
...
class AboutTab(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        _lbl(badge, "v1.1.0", 12, color=("gray70", "gray70")).pack(padx=12, pady=6)
        ...
        _lbl(self,
             ...
             13, color=("gray50", "gray50"), justify="left"
        ...
        for i, (text, url) in enumerate(links):
            if url:
                ...
                btn.grid(row=i+3, column=0, sticky="w", padx=22, pady=1)
            else:
                _lbl(self, text, 13, color=("gray50", "gray50")).grid(
                    row=i+3, column=0, sticky="w", padx=28, pady=1)
...
class TelegramExtractorApp:
    ...
    def _load_icon(self):
        ...
        try:
            if sys.platform == "win32" and os.path.exists(ico):
                ...
                self.root.iconphoto(True, ImageTk.PhotoImage(Image.open(png)))
        except Exception as e:
            print(f"Icon: {e}")
    ...
    def _build(self):
        ...
        # Title (no logo image — centred text)
        title_frame = ctk.CTkFrame(sb, fg_color="transparent")
        title_frame.grid(row=0, column=0, sticky="ew", padx=0, pady=(22, 16))
        _lbl(title_frame, "Get Telegram API", 13, bold=True,
        ...
        self.data_tab = DataTab(cont)
        self.main_tab = MainTab(cont, self.extractor, self._on_ready)
        self.settings_tab = SettingsTab(cont)
        self.about_tab = AboutTab(cont)

        self._tabs = {
            "main": self.main_tab, "data": self.data_tab,
            "settings": self.settings_tab, "about": self.about_tab,
        }
